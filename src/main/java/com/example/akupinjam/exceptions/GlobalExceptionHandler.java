...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ResponseDto> handleAuthenticationException(AuthenticationException ex, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                .body(new ResponseDto(401, "failed", "Unauthorized access", null));
    }

    // Handle Resource Not Found (404)
    @ExceptionHandler(ResourceNotFoundException.class)
    @ResponseStatus(HttpStatus.NOT_FOUND)
    public ResponseEntity<ResponseDto> handleResourceNotFound(ResourceNotFoundException ex, HttpServletRequest request) {
    ...
    public ResponseEntity<ResponseDto> handleBadRequest(IllegalArgumentException ex, HttpServletRequest request) {
    ...
    public ResponseEntity<ResponseDto> handleGenericException(Exception ex, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(new ResponseDto(500, "failed", "An unexpected error occurred", null));
    }
}
